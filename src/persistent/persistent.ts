...
export class Persistent {
	...
	private toReferenceObj( prop: PersistentProperty, rootCollections: Collections ) {
		...
		if ( Array.isArray( propValue ) ) {
			if ( prop.cachedProps && !prop.searchableArray ) throw new Error( `${ this.className }.${ prop.name } is an array but cached array props are only allowed in searchable array references` )
