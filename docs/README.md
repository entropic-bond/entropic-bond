...
You can find the API documentation in the [docs/](_media/docs) directory.
...
> See the complete example at [samples/01-persistence.ts](_media/01-persistence.ts)
...
> See the complete example at [samples/02-observability.ts](_media/02-observability.ts)
...
> See the complete example at [samples/03-auth.ts](_media/03-auth.ts)
...
> See the complete example at [samples/04-server-auth.ts](_media/04-server-auth.ts)
...
> See the complete example at [samples/05-cloud-storage.ts](_media/05-cloud-storage.ts)
...
> See the complete example at [samples/06-cloud-functions.ts](_media/06-cloud-functions.ts)
...
> See the complete example at [samples/07-realtime-listeners.ts](_media/07-realtime-listeners.ts)
...
> See the complete example at [samples/10-datasource-plugin.ts](_media/10-datasource-plugin.ts)
...
> See the complete example at [samples/08-cached-props.ts](_media/08-cached-props.ts)
...
> See the complete example at [samples/09-utils.ts](_media/09-utils.ts)

### Samples

Complete, runnable examples are available in the [`samples/`](_media/samples) directory:

| Section | Sample |
|---------|--------|
| Persistence | [01-persistence.ts](_media/01-persistence.ts) |
| Observability | [02-observability.ts](_media/02-observability.ts) |
| Auth | [03-auth.ts](_media/03-auth.ts) |
| Server Auth | [04-server-auth.ts](_media/04-server-auth.ts) |
| Cloud Storage | [05-cloud-storage.ts](_media/05-cloud-storage.ts) |
| Cloud Functions | [06-cloud-functions.ts](_media/06-cloud-functions.ts) |
| Realtime listeners | [07-realtime-listeners.ts](_media/07-realtime-listeners.ts) |
| Cached property references | [08-cached-props.ts](_media/08-cached-props.ts) |
| Utility functions | [09-utils.ts](_media/09-utils.ts) |
| DataSource plugins | [10-datasource-plugin.ts](_media/10-datasource-plugin.ts) |
